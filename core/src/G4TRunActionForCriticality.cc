...
G4ThreadLocal std::ofstream G4TRunActionForCriticality::CriticalityFile;
G4ThreadLocal std::map<G4int,std::vector<G4double>>      G4TRunActionForCriticality::NewNeutronFissionEnergyList;
G4ThreadLocal std::map<G4int,std::vector<G4ThreeVector>> G4TRunActionForCriticality::NewNeutronFissionPositionsList;
G4ThreadLocal std::map<G4int,std::vector<G4ParticleMomentum>> G4TRunActionForCriticality::NewNeutronFissionMomDirecsList;
G4ThreadLocal unsigned long long int*    G4TRunActionForCriticality::FluxParticlePosX   ;
G4ThreadLocal unsigned long long int*    G4TRunActionForCriticality::FluxParticlePosY   ;
G4ThreadLocal unsigned long long int*    G4TRunActionForCriticality::FluxParticlePosZ   ;
G4ThreadLocal unsigned long long *       G4TRunActionForCriticality::FluxParticleEnergy ;
G4ThreadLocal std::map<G4int,std::map<G4String,unsigned long long int>> G4TRunActionForCriticality::ParticleProduction;
G4ThreadLocal std::map<G4int,std::map<unsigned int,G4double>> G4TRunActionForCriticality::VoxelsED_Total ;
G4ThreadLocal std::map<G4int,std::map<unsigned int,G4double>> G4TRunActionForCriticality::VoxelsED2_Total ;
G4ThreadLocal std::map<G4int,std::map<unsigned int,unsigned long long int>> G4TRunActionForCriticality::VoxelsNOfValues ;
G4ThreadLocal std::map<G4int,std::map<G4String,unsigned long long int>> G4TRunActionForCriticality::NOfValues;
G4ThreadLocal std::map<G4int,std::map<G4String,G4double>> G4TRunActionForCriticality::ED_Total ;
G4ThreadLocal std::map<G4int,std::map<G4String,G4double>> G4TRunActionForCriticality::ED2_Total ;
G4ThreadLocal G4int G4TRunActionForCriticality::rank, G4TRunActionForCriticality::thread, G4TRunActionForCriticality::DataID, G4TRunActionForCriticality::CurrentBatch, G4TRunActionForCriticality::NumberOfRanksThreads, G4TRunActionForCriticality::TotalEventNumber;
//G4ThreadLocal G4double G4TRunActionForCriticality::Keffective;
//G4ThreadLocal G4int G4TRunActionForCriticality::NumberOfFissionNeutrons, G4TRunActionForCriticality::NumberOfFission, G4TRunActionForCriticality::NumberOfCapture,
G4ThreadLocal G4double G4TRunActionForCriticality::EnergyEmittedPerThread, G4TRunActionForCriticality::ParticleSourceEnergy, G4TRunActionForCriticality::ExecutionTimeInMin, G4TRunActionForCriticality::OneEventExecutionTimeInMs;
G4ThreadLocal std::chrono::steady_clock::time_point G4TRunActionForCriticality::start, G4TRunActionForCriticality::end ;
#endif

//G4ThreadLocal unsigned int G4TRunActionForCriticality::StepCN;
//G4ThreadLocal G4double G4TRunActionForCriticality::StepEnergy;
//G4ThreadLocal G4String G4TRunActionForCriticality::StepRegion;
...
G4TRunActionForCriticality::G4TRunActionForCriticality()
{
...
G4TRunActionForCriticality::~G4TRunActionForCriticality(){
    ...
    //G4cout << "\n\n\n\n\n\n from function : " << "G4TRunActionForCriticality::~G4TRunActionForCriticality()"<< G4endl;
}

//G4Run* G4TRunActionForCriticality::GenerateRun(){ return new G4TRun;}

void G4TRunActionForCriticality::WriteMacroscopicCrossSection(){
...
void G4TRunActionForCriticality::BeginOfRunAction(const G4Run* aRun) {

    //std::cout << " from function : " << "\n\n\n\n\n\n G4TRunActionForCriticality::BeginOfRunAction()"<< std::endl;
    ...
    */
    ...
    NumberOfBatch = 10;
    NumberOfEventInBatch=TotalEventNumber/NumberOfBatch;

    //if(NumberOfBatch*NumberOfEventInBatch < TotalEventNumber){}
    //else if(NumberOfBatch*NumberOfEventInBatch > TotalEventNumber){}
...
// it call analysisMan->save(); G4TRunActionForCriticality::CreateResultFileFor_Seq_MT()
void G4TRunActionForCriticality::EndOfRunAction(const G4Run* aRun)
{
...
void G4TRunActionForCriticality::CreateThreadVoxelsResultsFiles(){
...
// for step and event accuracy calculation
void G4TRunActionForCriticality::CreateThreadRegionResultFile(){
...
void G4TRunActionForCriticality::TestResults(){
...
void G4TRunActionForCriticality::CreateSimulationDataFile(){
...
void G4TRunActionForCriticality::FillRegionStepHits(G4String StepRegion, G4double StepEnergy){
...
void G4TRunActionForCriticality::FillVoxelStepHits(unsigned int StepCN, G4double StepEnergy){
    //G4cout << N << " " << E << G4endl;
...
void G4TRunActionForCriticality::pushVoxelEnergy(std::map<unsigned int,G4double>EventMap){
...
void G4TRunActionForCriticality::pushVoxelEnergy2(std::map<unsigned int,G4double>EventMap){
...
void G4TRunActionForCriticality::pushVoxelHitsNumber(std::map<unsigned int,unsigned int>EventMap){
...
// for GDML geometry
// Step Level accuracy calculation
void G4TRunActionForCriticality::pushVolumeEnergy(std::map<G4String,G4double>StepsMap){
...
void G4TRunActionForCriticality::pushVolumeEnergy2(std::map<G4String,G4double>StepsMap){
    for ( auto it = StepsMap.begin(); it != StepsMap.end(); ++it  ){
...
void G4TRunActionForCriticality::pushVolumeHitsNumber(std::map<G4String, unsigned int> StepsMap){
    for ( auto it = StepsMap.begin(); it != StepsMap.end(); ++it  ){
...
void G4TRunActionForCriticality::CountFission() {
    //G4cout << "Fission" << G4endl;
    //NumberOfFission++;
    FissionCapturesOfThreadsRanks[DataID][CurrentBatch][2]++ ;
}
void G4TRunActionForCriticality::CountAbsorption() {
    //G4cout << "Capture" << G4endl;
    //NumberOfCapture++;
    FissionCapturesOfThreadsRanks[DataID][CurrentBatch][1]++;
}
void G4TRunActionForCriticality::SetPoxEneForBatch(G4double X,G4double Y,G4double Z,G4double MX,G4double MY,G4double MZ,G4double E) {
...
void G4TRunActionForCriticality::CountParticleProductionByNeutron(G4String n){
    ParticleProduction[DataID][n]++;
}

void G4TRunActionForCriticality::IntializeBatchData() {
    ...
    if(CurrentBatch == NumberOfBatch && CurrentBatch*NumberOfEventInBatch >= TotalEventNumber){
        ...
        if(WriteData == true){
            ...
            CriticalityFile << "NumberOfRanksThreads       "    << NumberOfRanksThreads << "\n";
            ...
            //G4int max = 0;
            //for(G4int ll = 0 ;ll < NumberOfRanksThreads ;ll++){
            //    max += NewNeutronFissionEnergyList[ll].size();
            //}
            ...
            CriticalityFile << "SimulatedNeutronSourceData ("<< NumberOfRanksThreads*NumberOfEventInBatch <<") E X Y Z MX MY MZ\n";
            ...
            for(G4int ll = 0 ; ll < NumberOfRanksThreads ;ll++){
                G4cout << " NumberOfEventInBatch " <<NumberOfEventInBatch << " Begin from " << TotalEventNumber-NumberOfEventInBatch << G4endl;
                for( G4int nn = TotalEventNumber-NumberOfEventInBatch ; nn < TotalEventNumber ; nn++ ){
                    //CriticalityFile << NewNeutronFissionEnergyList   [ll][nn]       << " "
                    //                << NewNeutronFissionPositionsList[ll][nn].getX()<< " "
                    //                << NewNeutronFissionPositionsList[ll][nn].getY()<< " "
                    //                << NewNeutronFissionPositionsList[ll][nn].getZ()<< " "
                    //                << NewNeutronFissionMomDirecsList[ll][nn].getX()<< " "
                    //                << NewNeutronFissionMomDirecsList[ll][nn].getY()<< " "
                    //                << NewNeutronFissionMomDirecsList[ll][nn].getZ()<< "\n"  ;
                    CriticalityFile << EnergyListForCriticality   [ll][nn]       << " "
                                    << PositionsListForCriticality[ll][nn].getX()<< " "
                                    << PositionsListForCriticality[ll][nn].getY()<< " "
                                    << PositionsListForCriticality[ll][nn].getZ()<< " "
                                    << MomDirecsListForCriticality[ll][nn].getX()<< " "
                                    << MomDirecsListForCriticality[ll][nn].getY()<< " "
                                    << MomDirecsListForCriticality[ll][nn].getZ()<< "\n";
...
void G4TRunActionForCriticality::SetPoxEneOfFluxParticle(G4double X,G4double Y,G4double Z,G4double MX,G4double MY,G4double MZ,G4double E) {
    //G4cout << "Capture" << G4endl;
}
...
void G4TRunActionForCriticality::useTime(G4int set_get_for)
{
    if( set_get_for == 0){
        ...
        std::cout << DataID << " Events Simulation time was: " << timeConsumption * unitFactor << unit << ".";  // You could try using \r and \b. The first moves the cursor back to the start of the line, the latter back one character. Neither erase the exisiting text, so you've got to supply enough chars to erase all the old ones.

...
/*Convert( G4String fileName )
    G4tgrFileIn::GetWordsInLine(wl);
    CheckNColumns(wl,2);
    SetCompression(wl[1]);
    ...
    AddCT2Density(wl);
    AddFile(wl[1]);
        DicomFileCT*df::ReadData()
        DicomFileCT*df::SetFileName(fileName);
        theCTFiles[df->GetMaxZ()] = df;
        DicomFilePET*df::ReadData()
        DicomFilePET*df::SetFileName(fileName);
        thePETFiles[df->GetMaxZ()] = df;
    ProcessFiles();
        CheckCTSlices();
            DicomFileCT* and its get methods
        BuildCTMaterials();
        MergeCTFiles();
        CheckPETSlices();
            DicomFilePET* and its get methods
        BuildPETActivities();
        MergePETFiles();
        DumpToTextFile();
...
void DicomFileMgr::Convert( G4String fileName )
{
    ...
    for( ii = 0;; ii++) {
        ...
        if( wl[0] == ":COMPRESSION" ) {
            ...
            G4Exception("DICOM2G4", "Wrong argument", FatalErrorInArgument, G4String("UNKNOWN TAG IN FILE  "+wl[0]).c_str());
        }
...
void DicomFileMgr::CheckNColumns(std::vector<G4String> wl, size_t vsizeTh )
{
    if( wl.size() != vsizeTh ) {
        ...
        G4cerr << G4endl;
        G4Exception("DICOM2G4", "D2G0010", FatalErrorInArgument, ("Wrong number of columns in line " + std::to_string(wl.size()) + " <> " + std::to_string(vsizeTh)).c_str());
    }
...
void DicomFileMgr::AddFile( G4String fileName )
{
    ...
    if( ! (dfile.loadFile(fileName.c_str())).good() ) {
        G4Exception("DicomHandler::ReadFile", "dfile.loadFile", FatalErrorInArgument, ("Error reading file " + fileName).c_str());
    }
    ...
    if( dModality == "CT" ) {
        ...
        if(FileInc == 0){ // for all dcm files of a serie, then, get it just one time.
            G4cout << "\nModality: " <<dModality << " FileInc: " << FileInc <<  G4endl;
            df->ReadPETGeneralData();
            PETGeneralData = df->getPETGeneralData();
            PETGeneralDateData = df->getPETGeneralDateData();
            SerieAcquisitionsDataVec = df->getSerieAcquisitionsData();
        }
        ...
        G4Exception("DicomFileMgr::AddFIle()", "DFM001", FatalErrorInArgument, (G4String("File is not of type CT or RTSTRUCT or RTPLAN, but: ") + dModality).c_str());
    }

}


// called while the file reading
void DicomFileMgr::AddMaterial( std::vector<G4String> wl ){

    if( theMaterials.size() > 0 && bMaterialsDensity ) {
        G4Exception("DicomFileMgr::AddMaterial", "DFM005", FatalException, "Trying to add a Material with :MATE and another with :MATE_DENS, check your input file");
    }
...
G4double DicomFileMgr::Hounsfield2density(Uint32 Hval)
{
    ...
    if( G4int(Hval) < minHval ) {
        G4Exception("DicomHandler::Hounsfield2density", "", FatalException, ("Hval value too small, change input file "+std::to_string(Hval) + " < " + std::to_string(minHval)).c_str());
    }

    ite = theCT2Density.end(); ite--;
    G4int maxHval = (*ite).first;
    if( G4int(Hval) > maxHval ) {
        G4Exception("DicomHandler::Hval2density", "", FatalException, ("Hval value too big, change CT2Density.dat file "+std::to_string(Hval) + " > " + std::to_string(maxHval)).c_str());
    }
...
size_t DicomFileMgr::GetMaterialIndex( G4double Hval)
{
    ...
    if( ite == theMaterials.end() ) {
        ite--;
        G4Exception("DicomFileMgr::GetMaterialIndex", "DFM004", FatalException, ("Hounsfiled value too big, change input file "+std::to_string(Hval) + " > " + std::to_string((*ite).first)).c_str());
    }
...
{    
    ...
    if( ite == theMaterialsDensity.end() ) {
        ite--;
        G4Exception("DicomFileMgr::GetMaterialIndexByDensity", "DFM003", FatalException, ("Density too big, change input file "+std::to_string(density) + " > " + std::to_string((*ite).first)).c_str());
    }

    size_t dist = std::distance( theMaterialsDensity.begin(), ite );
    //G4cout << "D="<< density << " " << ite->first << " " << ite->second << " " << dist << G4endl; //GDEB
...
void DicomFileMgr::ProcessFiles()
{
    if( theCTFiles.size() == 0 ) {
        ...
        G4cout << "\nPROCESSING "<< theCTFiles.size() <<" CT files " << G4endl; //GDEB
    ...
    if( thePETFiles.size() != 0 ) {
        ...
        G4cout << "\nPROCESSING "<< thePETFiles.size() <<" PET files " << G4endl; //GDEB
...
void DicomFileMgr::CheckCTSlices()
{
    ...
    G4cout << " DicomFileMgr::Checking "<< nSlices <<" CT slices: " << G4endl;
...
void DicomFileMgr::CheckPETSlices()
{
    ...
    G4cout << " DicomFileMgr::Checking "<< nSlices <<" PET slices: "<< G4endl;
...
void DicomFileMgr::BuildCTMaterials(){
    ...
    G4cout << " DicomFileMgr::Building Materials for CT" << G4endl;//GDEB
    mdct::const_iterator ite = theCTFiles.begin();
...
void DicomFileMgr::BuildPETActivities(){
    ...
    G4cout << " DicomFileMgr::Building Activities for PET" << G4endl;//GDEB
    mdpet::const_iterator ite = thePETFiles.begin();
...
{
    G4cout << " DicomFileMgr::Merging "<< theCTFiles.size() <<" CT files " << G4endl;//GDEB
    mdct::const_iterator ite = theCTFiles.begin();
...
{
    G4cout << " DicomFileMgr::Merging "<< thePETFiles.size() <<" PET files " << G4endl;//GDEB
    mdpet::const_iterator ite = thePETFiles.begin();
...
void DicomFileMgr::DumpToTextFile()
{
    ...
    if( theCTFiles.size() != 0 ) {
        ...
        G4cout << "\n\n========= Dump Header and Materials ID and Density Data with Compression Value "<< fCompression<< " To Text File " << theFileOutName << " ======================= \n"<<G4endl;
    ...
    if( thePETFiles.size() != 0 ) {
        ...
        G4cout << "\n\n========= Dump Header and Acticity Data with Compression Value "<< fCompression<< " To Text File " << theFileOutName << " ======================= \n"<<G4endl;
...
// ////// added by ElTarik
void DicomFileMgr::GenerateDICOMTextGeometryFile(){

    GetInputData();

    for( G4int d = 0; d < DicomFilePathsVector.size() ;d++){
        FileInc = d;
        AddFile(DicomFilePathsVector[d]);
    }

    ProcessFiles();
    DicomFilePathsVector.clear();
    thePETFiles.clear();
    theCTFiles.clear();

}
void DicomFileMgr::GetInputData(){
    ...
    //theFileOutName = TConstruction0->getDicomOutTextName();

    DicomFilesDirPath = DicomFilesDirPath+"/*";

    //G4cout << " From function : " << __FUNCTION__ << " : " << DicomFilesDirPath << G4endl;

    GetDicomFilesNames(DicomFilesDirPath);

    G4cout << "\n" << std::setw(15) << std::left <<"CT" << std::setw(15) << std::left << "Density" << G4endl;
    for( G4int d = 0; d < ValueCT.size() ;d++){
        theCT2Density[ValueCT[d]] = ValueDensity[d];
        G4cout << std::setw(15) << std::left <<  ValueCT[d] << std::setw(15) << std::left << ValueDensity[d] << G4endl;
    }

    G4cout << "\n" << std::setw(15) << std::left <<"Density" << std::setw(15) << std::left << "Material name" << G4endl;
    bMaterialsDensity = true;
    for ( auto it = fMaterialIndices.begin(); it != fMaterialIndices.end(); ++it  ){
        theMaterialsDensity[it->first] += it->second ;
        G4cout << std::setw(15) << std::left <<it->first << std::setw(15) << std::left << it->second << G4endl;
    }

}
#include <glob.h>
std::vector<G4String> DicomFileMgr::GetDicomFilesNames(G4String DirPath)
{

    G4cout << " The .dcm DICOM Files to be processed: " << G4endl;
    ...
    for ( unsigned int i=0 ; i < glob_result.gl_pathc ; ++i ){
        ...
        if(vvv.contains(".dcm")){
            G4cout << i << " " << glob_result.gl_pathv[i] << G4endl;
            DicomFilePathsVector.push_back(glob_result.gl_pathv[i]);
            //G4cout << " size : " << DicomFilePathsVector.size() << G4endl;
        }
    }

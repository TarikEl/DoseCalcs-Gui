...
G4TKleinNishinaCompton::G4TKleinNishinaCompton(const G4ParticleDefinition*, const G4String& nam) : G4VEmModel(nam)
{
...
G4TKleinNishinaCompton::~G4TKleinNishinaCompton(){}


void G4TKleinNishinaCompton::Initialise(const G4ParticleDefinition* p,
                                        const G4DataVector& cuts)
{
    //G4cout << "\n\n\n\n\n\n\n\n\n\n\n void G4TKleinNishinaCompton::Initialise()" << G4endl;
...
//void G4TKleinNishinaCompton::StartTracking(G4Track* t) {track = t;}

void G4TKleinNishinaCompton::InitialiseLocal(const G4ParticleDefinition*, G4VEmModel* masterModel)
{ SetElementSelectors(masterModel->GetElementSelectors()); }



G4double G4TKleinNishinaCompton::ComputeCrossSectionPerAtom( const G4ParticleDefinition*, G4double GammaEnergy, G4double Z, G4double, G4double, G4double)
{
...
void G4TKleinNishinaCompton::SampleSecondaries( std::vector<G4DynamicParticle*>* fvect,const G4MaterialCutsCouple*,
                                                const G4DynamicParticle* aDynamicGamma, G4double,G4double)
{
    ...
    if(gamEnergy1 > lowestSecondaryEnergy) {
        ...
        // new by me
        //fParticleChange->SetParentWeightByProcess(fParticleChange->GetParentWeight()/SplittingNum);
        //G4cout << "Change weigh from kleinNishinaModel to " << fParticleChange->GetParentWeight()/SplittingNum << G4endl;

        if(fParticleChange->GetWeight() == 1) {


            //std::cout << " Weight="<< fParticleChange->GetWeight() << " ParentWeight="<< fParticleChange->GetParentWeight() << std::endl;

            //if(G4UniformRand() > 0.2){

            G4DynamicParticle* dpg = new G4DynamicParticle(theGamma,gamDirection1,gamEnergy1);
            fvect->push_back(dpg);
            fParticleChange->ProposeWeight(0.2);
    ...
    if(eKinEnergy > lowestSecondaryEnergy) {
        ...
        //G4cout << "\n\n\n\n\n\n\n\n\n\n\n\n from G4TKleinNishinaCompton::SampleSecondaries() --- trackID" << track->GetParentID() << G4endl;

        //
        //for (G4int hj = 0; hj < 1 ; hj++) {
        //fvect->push_back(dpg);
        //}

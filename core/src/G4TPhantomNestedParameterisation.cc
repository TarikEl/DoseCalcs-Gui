...
G4TPhantomNestedParameterisation::G4TPhantomNestedParameterisation(const G4ThreeVector& halfVoxelSize,std::vector<G4Material*>& mat, G4int fnX_, G4int fnY_, G4int fnZ_)
    :
...
G4TPhantomNestedParameterisation::~G4TPhantomNestedParameterisation()
{}

void G4TPhantomNestedParameterisation::ReadColourData()
{
...
void G4TPhantomNestedParameterisation::SetNoVoxel( G4int nx, G4int ny, G4int nz )
{
    fnX = nx;
    fnY = ny;
    fnZ = nz;
...
G4Material* G4TPhantomNestedParameterisation::ComputeMaterial(G4VPhysicalVolume* physVol, const G4int iz, const G4VTouchable* parentTouch)
{
    ...
    //G4cout << "copyNo from the Nested Param: "<< copyNo << G4endl;

    //The copyNo identifies the voxel
    std::size_t matIndex = GetMaterialIndex(copyNo);
    static G4Material* mate = nullptr;

    //std::cout << " ix " << ix << " iy " << iy << " iz " << iz << " copyNo from the Nested Param: " << copyNo <<  " MaterialIndex: " << matIndex << std::endl;
    ...
    if(true && physVol && G4VVisManager::GetConcreteInstance()) {
        ...
        if(0 < fColours.count(mateName)) {
            ...
            physVol->GetLogicalVolume()->SetVisAttributes(fColours.begin()->second); // Associate color
        }
    }
    */
...
G4int G4TPhantomNestedParameterisation::GetMaterialIndex( G4int copyNo ) const
{
    return fMaterialIndices[copyNo];
}

G4int G4TPhantomNestedParameterisation::GetNumberOfMaterials() const
{
    return fMaterials.size();
}

G4Material* G4TPhantomNestedParameterisation::GetMaterial(G4int i) const
{
    return fMaterials[i];
}

void G4TPhantomNestedParameterisation::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const
{
    // Position of voxels.
...
void G4TPhantomNestedParameterisation::ComputeDimensions( G4Box& box, const G4int, const G4VPhysicalVolume* ) const
{
